...
def parseinput(file_name):
    file=open(file_name)
    dflist=file.readlines()
    edgesforgraph=[]
    listofnames=[]
    for i in dflist:
        i=i.rstrip()
        rowlist=i.split(sep=',')
        listofnames.append([rowlist[0],rowlist[1]])
        rowlist=[eval(j) for j in rowlist[2:]]
        maxelem=max(rowlist)
        number_of_vertices=ceil(sqrt(maxelem+1))
        l=[]
        for elem in rowlist:
            idx1=elem//number_of_vertices
            idx2=elem%number_of_vertices
            t=(idx1,idx2)
            l.append(t)
        edgesforgraph.append(l)
    return listofnames,edgesforgraph
...
_all_ = ["closeness_centrality", "incremental_closeness_centrality"]
...
def Driver(filename):
    nameslist,edgeslist=parseinput(filename)
    currentedgelist=[]
    writer = csv.writer(open("output.csv", 'w'))
    for i in range(len(nameslist)):
        currentedgelist.extend(edgeslist[i])
        G=nx.Graph(currentedgelist)
        cc_dict = nx.closeness_centrality(G)
        row = [nameslist[i][0],nameslist[i][1]]
        for key in cc_dict:
            row.append(cc_dict[key])
        writer.writerow(row)

Driver(filename)

# Call driver to run the program
